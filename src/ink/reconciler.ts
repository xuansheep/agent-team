...
>({
  ...
  prepareUpdate(
    ...
    return {
      props,
      style,
      nextStyle: newProps['style'] as Styles,
    }
  },
  createInstance(
  ...
  commitUpdate(
    ...
    const payload =
      typeof updatePayloadOrType === 'string'
        ? {
            props: diff(typeOrOldProps as Props, oldPropsOrNewProps),
            style: diff(
              (typeOrOldProps as Props)['style'] as Styles,
              oldPropsOrNewProps['style'] as Styles,
            ),
            nextStyle: oldPropsOrNewProps['style'] as Styles,
          }
        : updatePayloadOrType
    ...
    if (payload.style && node.yogaNode) {
      applyStyles(node.yogaNode, payload.style, payload.nextStyle)
    }
  },
