...
export const CONTEXT_DISPLAY_BASELINE_TOKENS = 12_000;
...
export function contextUsedPercent(contextTokens: number, effectiveContextWindow: number): number {
  const used = Math.max(0, contextTokens - CONTEXT_DISPLAY_BASELINE_TOKENS);
  const effectiveCapacity = Math.max(1, effectiveContextWindow - CONTEXT_DISPLAY_BASELINE_TOKENS);
  return Math.min(100, Math.max(0, Math.round((used / effectiveCapacity) * 100)));
}
