...
describe("TUI event adapter", () => {
  ...
  it("keeps the plan document visible and switches to revision input after pausing", () => {
    ...
    state = reduceStoredEvent(state, {
      ...
      questions: [{ id: "next_step", text: "请继续输入修改意见", required: true }],

      ts: "2026-06-23T00:00:02.500Z",
    ...
    assert.deepEqual(state.questions, [{ id: "next_step", text: "请继续输入修改意见", required: true }]);
