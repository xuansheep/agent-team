...
export const nodeResultOutputInstructions = [
  ...
  "Use direction forward when the next workflow node should continue, backward when the previous workflow node must rework or answer a question, and retry only when the current node itself must continue working.",
  ...
  "When retrying the current node, include concrete defects/change_requests in feedback and a clear self-retry instruction in handoff; retry must not contain user questions.",
...
export const nodeResultSchema = z.object({
  ...
  direction: z.enum(["forward", "backward", "retry"]),
...
function looksLikeNodeResult(value: unknown): boolean {
  ...
  return direction === "forward" || direction === "backward" || direction === "retry";
