...
import {
  flushReconcilerSyncCompat,
  updateContainerSyncCompat,
} from './reconciler-compat.js'
import {
...
export default class Ink {
  ...
  private exitState:
    | { status: 'pending' }
    | { status: 'resolved' }
    | { status: 'rejected'; error: Error } = { status: 'pending' }
  private waitUntilExitKeepAlive?: ReturnType<typeof setInterval>
  ...
  render(node: ReactNode): void {
    ...
    updateContainerSyncCompat(tree, this.container)
  }
  ...
  unmount(error?: Error | number | null): void {
    ...
    if (this.waitUntilExitKeepAlive) {
      clearInterval(this.waitUntilExitKeepAlive)
      this.waitUntilExitKeepAlive = undefined
    }
    ...
    updateContainerSyncCompat(null, this.container)
    ...
    if (error instanceof Error) {
      this.exitState = { status: 'rejected', error }
      this.rejectExitPromise(error)
    } else {
      this.exitState = { status: 'resolved' }
      this.resolveExitPromise()
    }
  }

  async waitUntilExit(): Promise<void> {
    if (this.exitState.status === 'rejected') {
      throw this.exitState.error
    }

    if (this.exitState.status === 'resolved') {
      return
    }

    this.waitUntilExitKeepAlive ??= setInterval(noop, 60_000)
