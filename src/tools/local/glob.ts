...
export const globTool: Tool = {
  ...
  async execute(input, context) {
    ...
    assertGlobInsideWorkspace(context.cwd, parsed.pattern);
    ...
    const inside = matches.filter((match) => isPathInsideOrSame(context.cwd, resolve(context.cwd, match)));
    return { output: inside.join("\n") };
  }
};
