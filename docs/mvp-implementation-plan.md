...
**技术栈：** TypeScript、Node.js、npm、node:test、Zod、Commander、js-yaml、fast-glob、undici、OpenAI-compatible Chat Completions API。
...
- devDependencies 包含 `typescript`、`@types/node`、`@types/js-yaml`
...

{
  ...
  "scripts": {
    ...
    "test": "npm run build:test && node scripts/run-tests.mjs",
...
写入 `tsconfig.json`，使用 `target: ES2022`、`module: NodeNext`、`moduleResolution: NodeNext`、`strict: true`、`outDir: dist`，并包含 `src/**/*.ts`。另写 `tsconfig.test.json` 用于编译 `src/**/*.ts` 和 `tests/**/*.ts` 到 `dist-test`。

- [ ] **步骤 3：创建 node:test 运行配置**

写入 `tsconfig.test.json`，用于把 `src/**/*.ts` 和 `tests/**/*.ts` 编译到 `dist-test`。写入 `scripts/run-tests.mjs`，递归导入 `dist-test/tests/**/*.test.js`，由 Node 内置 `node:test` 执行测试。
