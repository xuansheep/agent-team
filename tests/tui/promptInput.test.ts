...
describe("PromptInput core logic", () => {
  ...
  it("maps terminal keys to prompt actions", () => {
    ...
    assert.equal(resolvePromptKey("", { return: true, shift: true }), "newline");
    assert.equal(resolvePromptKey("", { return: true, ctrl: true }), "newline");
    assert.equal(resolvePromptKey("", { return: true, meta: true }), "none");
  ...
  it("suggests slash commands and workflow targets", () => {
    ...
    assert.deepEqual(slashCommandSuggestions("/d", ["delivery"]), []);
