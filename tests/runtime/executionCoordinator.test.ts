...
describe("ExecutionCoordinator", () => {
  ...
  it("reuses an existing workflow after Plan approval and rolls back a failed dispatch", async () => {
    const cwd = await workspace("agent-team-coordinator-existing-workflow-");
    const store = new SessionStore(join(cwd, ".session"));
    const base = createKernelSession({
      id: "session-existing-workflow",
      cwd,
      permissions: { mode: "default", allow: [], ask: [], deny: [] },
      messages: [{ role: "user", content: "plan a safe change" }]
    });
    const planning = new PlanModeController().enterPlanMode(base, { request: "plan a safe change" });
    assert.ok(planning.planState);
    await writePlan(planning.planState.planFilePath, "# Plan\n\nImplement safely.\n");
    const provider: ModelProvider = {
      async generate() {
        return { content: "Ready.", tool_calls: [{ id: "exit-existing", name: "ExitPlanMode", input: {} }] };
      }
    };
    const coordinator = new ExecutionCoordinator(undefined, { sessionStore: store });
    const approval = await coordinator.executeSession({
      session: planning,
      provider,
      model: "test-model",
      tools: createLocalToolRegistry()
    });
    assert.equal(approval.session.status, "waiting_plan_approval");

    const failedWorkflow = {
      sessionId: approval.session.id,
      runId: "run-failed-dispatch",
      state: {
        status: "awaiting_bus",
        workflow_id: "main",
      ...
      async dispatchToNode() {
        throw new Error("dispatch failed");
      },
      result: new Promise(() => {})
    } as unknown as WorkflowSession;
    await assert.rejects(
      coordinator.resolvePlanApprovalAndDispatch({
    ...
    const rolledBack = await store.restoreKernelSession(approval.session.id, cwd);
    assert.equal(rolledBack?.status, "waiting_plan_approval");
    assert.equal(rolledBack?.pendingInteraction?.type, "plan_approval");

    let capturedInput: unknown;
    let capturedOptions: unknown;
    const workflowState = {
      status: "awaiting_bus" as const,
      workflow_id: "main",
      current_node_id: "product",
      attempts: []
    };
    const workflow = {
      sessionId: approval.session.id,
      runId: "run-existing-approved",
      state: workflowState,
      async dispatchToNode(nodeId: string, input: unknown, options: unknown) {
        capturedInput = input;
        capturedOptions = options;
        Object.assign(workflowState, { status: "running", current_node_id: nodeId });
      },
      result: new Promise(() => {})
    } as unknown as WorkflowSession;

    const transition = await coordinator.resolvePlanApprovalAndDispatch({
      session: approval.session,
      config: {} as AgentTeamConfig,
      workflowId: "main",
      workflow,
      startNodeId: "dev",
      permissionMode: "fullAccess",
      reason: "Resume approved plan"
    });

    assert.equal(transition.workflow, workflow);
    assert.equal(transition.resolution.session.status, "running_workflow");
    assert.equal(transition.resolution.session.workflowBinding?.runId, workflow.runId);
    assert.equal((capturedInput as { approved_plan?: unknown }).approved_plan, "# Plan\n\nImplement safely.");
    assert.deepEqual(capturedOptions, {
      reason: "Resume approved plan",
      permissionMode: "fullAccess"
    });
    const metadata = await store.loadMetadata(approval.session.id);
    assert.equal(metadata?.currentRunId, workflow.runId);
    assert.equal(metadata?.execution?.workflowBinding?.runId, workflow.runId);
  });
