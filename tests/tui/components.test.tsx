  ...
  it("renders workflow nodes with model names, English statuses, and an active running border", () => {
    ...
    const output = render(
      ...
      <WorkflowFlowChart
        ...
        nodes={[{ nodeId: "dev", attempt: 1, contextTokens: 100000, status: "running" }, { nodeId: "product", attempt: 1, status: "success" }]}
...
describe("InteractionArea", () => {
  ...
  it("renders activity status above the prompt when no choice is active", () => {
    ...
    const frame = output.lastFrame() ?? "";
    assert.match(frame, /• Working \(12s • esc to interrupt\) ─+/);
    assert.match(frame, /> Type a request or \/help/);
    assert.ok(frame.indexOf("Working (12s • esc to interrupt)") < frame.indexOf("> Type a request or /help"));
  ...
  it("renders conversation interruption as a warning activity row", () => {
    const output = render(
      <InteractionArea
        ...
        questions={[]}
        isLoading={false}
        activityStatus={{ kind: "warning", text: CONVERSATION_INTERRUPTED_TEXT }}
        onPromptEvent={() => undefined}
      />
    );

    const frame = output.lastFrame() ?? "";
    assert.match(frame, /■ Conversation interrupted[.] Describe how to proceed.*─+/);
  ...
  it("keeps activity status out of active choice layouts", () => {
    ...
    assert.doesNotMatch(frame, /Working \(12s • esc to interrupt\)/);
...
describe("TuiApp", () => {
  ...
  it("keeps the prompt as the bottom interaction area in the running layout", () => {
    ...
    assert.match(headerLine, /workflow delivery/);
    assert.doesNotMatch(headerLine, /agent-team/);
    assert.doesNotMatch(headerLine, /D:\\CodeAI\\agent-team/);
  ...
  it("pins configured workflow nodes above the prompt", () => {
    ...
    const config = {
      ...
      providers: {
        ...
        default: { type: "responses-api" as const, responses: { prompt_cache: true, parallel_tool_calls: true }, base_url: "https://api.example.test/v1", api_key: "test-key", default_model: "gpt-test", capabilities: { tool_calling: false, vision: false, streaming: false, json_schema_output: true } }
